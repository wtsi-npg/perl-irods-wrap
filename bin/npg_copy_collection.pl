...
try {
  WTSI::NPG::iRODS->new->copy_collection($source, $dest);
}
catch {
...
npg_copy_collection --source <path> --destination <path>
  [--debug] [--verbose]
...
Copy an iRODS collection recursively, including all contents and metadata.
